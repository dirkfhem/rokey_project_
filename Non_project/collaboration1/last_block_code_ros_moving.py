...
PLATE_POSITIONS = [
    ([497.04, 152.34, 100.24, 1.94, -179.83, 2.23], [599.04, 48.95, 100.24, 1.94, -179.83, 2.23]),  # 플레이트 0: 위치 0, 위치 8
    ([495.83, -49.11, 100.24, 1.94, -179.83, 2.23], [598.1, -151.73, 100.24, 1.94, -179.83, 2.23])   # 플레이트 1: 위치 0, 위치 8
]
...
class plate_control:
    ...
    def move_item(self, from_plate, from_idx, to_plate, to_idx):
        print(f"{from_plate},{from_idx}-{to_plate},{to_idx}")
        ...
        if (0 <= from_plate < self.num_plates and 0 <= to_plate < self.num_plates and
            ...
            # 목표 위치에 아이템 놓기
            movel(target_poly[POS_X], vel=DR_VEL_J, acc=DR_ACC_J)
    ...
    def sort_by_height(self, sort=DESC):
        ...
        # Update home plate
        self.plate[0] = temp_exist
        self.home_m_height = temp_heights
        self.exist_is[0] = temp_plate
