...
class ControlIntersectionSign(Node):
    ...
    def __init__(self):
        super().__init__('control_intersection_sign')  # 노드 이름 설정
        ...
        self.sub_intersection = self.create_subscription(
            Bool,                           # 메시지 타입: 참/거짓
            '/detect/intersection_sign',    # 토픽 이름
            self.cb_intersection_detected,  # 메시지 받았을 때 실행할 함수
            10                             # 큐 크기
        )
        ...
        # 초기화 완료 메시지
        self.get_logger().info('ControlIntersectionSign Node Initialized')
    ...
    def cb_intersection_detected(self, msg):
        """
        교차로 표지판 인식 콜백 함수
        - 교차로 표지판이 감지되면 호출
        ...
        if msg.data and self.can_process_detection():
            self.get_logger().info('🚦 Intersection sign detected! Starting intersection control sequence')
            self.control_state = ControlState.PROCESSING_INTERSECTION
    ...
    def control_loop(self):
        ...
        # 현재 상태에 따른 제어 함수 호출
        if self.control_state == ControlState.PROCESSING_INTERSECTION:
            self.handle_intersection_control(elapsed_time)
    ...
    def handle_intersection_control(self, elapsed_time):
        """
        교차로 제어 처리 함수
        - 1단계: 지정된 시간 동안 완전 정지
        - 2단계: 천천히 전진하며 교차로 통과
        ...
        if elapsed_time < self.stop_duration:
            ...
            if elapsed_time < 0.1:  # 첫 번째 제어 루프에서만 로그 출력
                self.get_logger().info(f'🛑 Stopping at intersection for {self.stop_duration} seconds')
                
        elif elapsed_time < self.stop_duration + 1.0:
            # 2단계: 천천히 직진하며 교차로 통과
            self.move_forward_slow()
            if elapsed_time < self.stop_duration + 0.1:
                self.get_logger().info('🚗 Proceeding through intersection slowly')
                
        else:
            # 3단계: 교차로 통과 완료, 정상 상태로 복귀
            self.get_logger().info('✅ Intersection sequence completed')
    ...
    def get_current_state_string(self):
        ...
        state_strings = {
            ...
            ControlState.PROCESSING_INTERSECTION: "Processing Intersection",
...
def main(args=None):
    ...
    try:
        node = ControlIntersectionSign()  # 교통 표지판 제어 노드 생성
