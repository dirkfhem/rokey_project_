...
class DetectLane(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("DetectLane node initialized")
        ...
        parameter_descriptor_white_balance = ParameterDescriptor(
            description='white balance range',
            integer_range=[IntegerRange(from_value=2000, to_value=10000, step=1)]
        )
        parameter_descriptor_exposure = ParameterDescriptor(
            description='exposure range',
            integer_range=[IntegerRange(from_value=-10, to_value=10, step=1)]
        )
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('detect.lane.yellow.hue_h', 60, parameter_descriptor_hue),
                ('detect.lane.yellow.saturation_l', 0, parameter_descriptor_saturation_lightness),
                ('detect.lane.yellow.saturation_h', 255, parameter_descriptor_saturation_lightness),
                ('detect.lane.yellow.lightness_l', 0, parameter_descriptor_saturation_lightness),
                ('detect.lane.yellow.lightness_h', 255, parameter_descriptor_saturation_lightness),
                ('is_detection_calibration_mode', False),
                ('preprocessing.saturation', 32, parameter_descriptor_saturation_lightness),
                ('preprocessing.white_balance', 4899, parameter_descriptor_white_balance),
                ('preprocessing.exposure', -7, parameter_descriptor_exposure),
            ]
        )
        ...
        try:
            ...
            self.saturation = self.get_parameter('preprocessing.saturation').get_parameter_value().integer_value
            self.white_balance = self.get_parameter('preprocessing.white_balance').get_parameter_value().integer_value
            self.exposure = self.get_parameter('preprocessing.exposure').get_parameter_value().integer_value
        ...
        if self.sub_image_type == 'compressed':
            ...
            self.get_logger().info("Subscribed to /detect/image_input/compressed")
        elif self.sub_image_type == 'raw':
            self.sub_image_original = self.create_subscription(
                Image, '/detect/image_input', self.cbFindLane, 1
            )
            self.get_logger().info("Subscribed to /detect/image_input")
    ...
    def preprocess_image(self, image):
        """소프트웨어적으로 preprocessing 설정 적용"""
        try:
            # 채도 조정
            hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
            hsv[:, :, 1] = cv2.convertScaleAbs(hsv[:, :, 1], alpha=self.saturation / 50.0)
            # 노출 조정 (밝기)
            hsv[:, :, 2] = cv2.convertScaleAbs(hsv[:, :, 2], alpha=1.0, beta=self.exposure * 10)
            # 화이트 밸런스 모의 (간단한 색온도 조정)
            bgr = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
            if self.white_balance != 0:
                temp_factor = self.white_balance / 6500.0  # 기준 6500K
                bgr = cv2.convertScaleAbs(bgr, alpha=temp_factor)
            return bgr
        except Exception as e:
            self.get_logger().error(f"Error in preprocess_image: {e}")
            return image

    def cbGetDetectLaneParam(self, parameters):
        for param in parameters:
            self.get_logger().info(f'Parameter name: {param.name}, value: {param.value}')
            ...
            elif param.name == 'preprocessing.saturation':
                self.saturation = param.value
            elif param.name == 'preprocessing.white_balance':
                self.white_balance = param.value
            elif param.name == 'preprocessing.exposure':
                self.exposure = param.value
        return SetParametersResult(successful=True)

    def auto_exposure(self, frame):
        yuv = cv2.cvtColor(frame, cv2.COLOR_BGR2YUV)
        yuv[:, :, 0] = cv2.equalizeHist(yuv[:, :, 0])  # Y 채널 평탄화
        return cv2.cvtColor(yuv, cv2.COLOR_YUV2BGR)

    def cbFindLane(self, image_msg):
        self.get_logger().info("cbFindLane called")
        if self.counter % 3 != 0:
            self.counter += 1
            return
        else:
        ...
        try:
            if self.sub_image_type == 'compressed':
                self.get_logger().info("Reading compressed image")
                np_arr = np.frombuffer(image_msg.data, np.uint8)
                cv_image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)
            elif self.sub_image_type == 'raw':
                self.get_logger().info("Reading raw image")
                cv_image = self.cvBridge.imgmsg_to_cv2(image_msg, 'bgr8')
                cv_image = self.auto_exposure(cv_image)

            # 노란색 차선용 프레임: 소프트웨어 전처리 적용
            cv_image_yellow = self.preprocess_image(cv_image.copy())

            white_fraction, cv_white_lane = self.maskWhiteLane(cv_image)
            self.get_logger().info(f"White lane fraction: {white_fraction}")
            yellow_fraction, cv_yellow_lane = self.maskYellowLane(cv_image_yellow)
            self.get_logger().info(f"Yellow lane fraction: {yellow_fraction}")
            ...
            try:
                ...
                self.get_logger().error(f"Error in lane fitting: {e}")
                if yellow_fraction > 3000:
    ...
    def make_lane(self, cv_image, white_fraction, yellow_fraction):
        ...
        if self.reliability_white_line > 50 and self.reliability_yellow_line > 50:
            if white_fraction > 432 and yellow_fraction > 432:
            ...
            if white_fraction > 432 and yellow_fraction <= 432:
                centerx = np.subtract(self.right_fitx, 100)
                pts_center = np.array([np.transpose(np.vstack([centerx, ploty]))])
            ...
            if white_fraction <= 432 and yellow_fraction > 432:
                centerx = np.add(self.left_fitx, 100)
                pts_center = np.array([np.transpose(np.vstack([centerx, ploty]))])
        ...
        if self.pub_image_type == 'compressed':
            ...
            if self.is_center_x_exist:
                msg_desired_center = Float64()
                msg_desired_center.data = centerx.item(min(240, centerx.size - 1))
                self.pub_lane.publish(msg_desired_center)
