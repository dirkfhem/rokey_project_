...
class YOLOWebcamProcessor:
    ...
    def run(self):
        ...
        while not self.should_shutdown:
            ...
            results = self.model(img_crop, stream=True)
            ...
            for r in results:
                ...
                for box in boxes:
                    ...
                    org = [x1, y1]
                    cv2.putText(img_crop, f"{label}: {confidence}", org, cv2.FONT_HERSHEY_SIMPLEX, fontScale, (255, 0, 0), 2)

                    self.csv_output.append([x1, y1, x2, y2, confidence, label])
                    object_count += 1

            self.max_object_count = max(self.max_object_count, object_count)                #최대 객체수
            cv2.putText(img_crop, f"Objects_count: {object_count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 255, 0), 1) # 이미지에 최종 개수 넣기

            if object_count > 0:                                            # 객체가 하나이상 탐지되면
                filename = f'output_{int(time.time())}.jpg' 
                cv2.imwrite(os.path.join(self.output_dir, filename), img_crop)   # 위에서 만든 사진 저장.

            display_img = cv2.resize(img_crop, (img_crop.shape[1]*2, img_crop.shape[0]*2)) # 보일 이미지
...
def main():
    model_path = '/home/minsuje/ros2_ws/runs/detect/train7/weights/best.pt'
